...
class Setting extends CI_Controller 
{
    ...
    {
        $data = array();
        $this->load->view('admin/menu/view_menu_hyperlink',$data);
    }

    public function statics()
    {
        $data = array();
        $this->load->view('admin/menu/view_menu_static',$data);
    }

    public function custom()
    {
        $data = array();
        $this->load->view('admin/menu/view_menu_custom',$data);
    }

    public function menu()
    {
        if(isset($_GET['request'])) {
            $data['parents'] = $this->Model_menu->listHierarchy();

            $this->load->view('admin/menu/view_menu_list',$data);
        } else {
            if(isset($_POST['itemId'])) {
                $i=0;
                $children = json_decode($_POST['children']);
                $parent = ($_POST['parentId'] == '') ? null : $_POST['parentId'];

                try {
                    foreach ($children as $child) {
                        $form_data = array();
                        if($child == $_POST['itemId']) {
                            $form_data['menu_parent'] = $parent;
                        }

                        $form_data['order'] = $i++;
                        $this->Model_menu->update($_POST['itemId'], $form_data);

                        $message = 'Menu successfully update';
                    }
                ...
                try {
                    $form_data = array(
                        ...
                        'menu_type_param'   =>  $_POST['menu_type_param'],
                    );
                    $this->Model_menu->add($form_data);
                    $message = 'Menu is added successfully!';
                } catch (ErrorException $e) {
                    $message = $e->getMessage();
                }
