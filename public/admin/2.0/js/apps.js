...
var handleAjaxMode = function(setting) {
	var emptyHtml = (setting.emptyHtml) ?  setting.emptyHtml : '<div class="p-t-40 p-b-40 text-center f-s-20 content"><i class="fa fa-frown fa-lg text-muted m-r-5"></i> <span class="f-w-600 text-inverse">Page cannot be found, it has disappeared from here</span></div>';
...
/* 30. Handle Notification - added in V4.1
------------------------------------------------ */
var handleNotification = function(title, message) {
    $.gritter.add({
        title: title,
        text: message
    });
...
/* 31. Handle Desktop Notification - added in V4.1
------------------------------------------------ */
var handleDesktopNotification = function(title, message) {
    if(!Notification) {
        alert('Desktop notification not available in your browser. Try Microsoft Edge.');
        return;
    }
...
/* 32. Handle Facebox - added in V4.1
------------------------------------------------ */
var handleLoad = function(url, div) {
    $.ajax({
        url: url,
        type: "get",
        dataType: "html",
...
var handleLoadIntoBox = function(url, div, param) {
    $.ajax({
        url: url,
        data: param,
        type: "get",
...
var handleConfirmPopUp = function(obj, command, title, text, yes, no, param) {
    $.facebox.confirm({
        title: title,
        text: text,
        button: {
...
var handleDelete = function (action) {
    $.ajax({
        url: action,
        data: {
            id: $(this).data('id')
...
var handleConfirmDirectPopUp = function(action, title, text, yes, no) {
    $.facebox.confirm({
        title: title,
        text: text,
        button: {
...
var handleAlertDirectPopUp = function(title, text, actionName, action) {
    $.facebox.confirm({
        title: title,
        text: text,
        button: {
	...
	return {
		...
		init: function (option) {
		    ...
		    $.ajaxSetup({
                statusCode: {
                    400: function() {
                        this.handleAlertDirectPopUp('Notification', 'Bad Request, Please try again.', 'OK', '')
                    },
                    401: function() {
                        this.handleAlertDirectPopUp('Notification', 'Please Log In to continue', 'Log In', '')
                    },
                    404: function() {
                        this.handleAlertDirectPopUp('Notification', 'Page cannot be found, it has disappeared from here', 'Close', '')
                    },
                    500: function() {
                        this.handleAlertDirectPopUp('Whoopps', 'There seems to be an error, please contact your administrator', 'OK', '')
                    }
                }
            });
        ...
        dekstopNotification: function (title, message) {
            handleDesktopNotification(title, message);
        },
        load: function (uri, div) {
            handleLoad(uri, div);
        },
        loadIntoBox: function (url, div, param) {
            handleLoadIntoBox(url, div, param);
        },
        confirmDirectPopUp: function (action, title, text, yes, no) {
            handleConfirmDirectPopUp(action, title, text, yes, no);
        },
        confirmPopUp: function (obj, command, title, text, yes, no, param) {
            handleConfirmPopUp(obj, command, title, text, yes, no, param);
        }
  };
}();